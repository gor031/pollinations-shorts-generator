# Pollinations Shorts Generator

> **AI로 생성한 이미지를 연결해 짧은 쇼츠 영상을 만들어보세요!**
>
> [![Built with Pollinations](https://img.shields.io/badge/Built%20with-Pollinations-8a2be2?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&labelColor=6a0dad)](https://pollinations.ai)

## 기능

- 🎨 **Pollinations AI 이미지 생성** API 활용
- 📱 **9:16 쇼츠 비율**에 최적화된 이미지 생성
- 🎬 **자동 슬라이드쇼 플레이어**로 쇼츠 미리보기
- 🖼️ **장면 수 조절** (3~10장)
- 💾 **개별 이미지 다운로드** 지원

## 기술 스택

- React + TypeScript + Vite
- Pollinations 이미지 생성 API

## 로컬 실행

```bash
npm install
npm run dev
```

## 빌드

```bash
npm run build
```

## 크레딧

- [pollinations.ai](https://pollinations.ai)
- Official Logos: [Logo White](https://raw.githubusercontent.com/pollinations/pollinations/main/assets/logo.svg) / [Logo Text White](https://raw.githubusercontent.com/pollinations/pollinations/main/assets/logo-text.svg)
